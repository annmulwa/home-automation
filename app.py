...
CREATE_TEMPS_TABLE = """CREATE TABLE IF NOT EXISTS temperatures (room_id INTEGER,
                        temperature REAL, date TIMESTAMP, FOREIGN KEY(room_id)
...
ROOM_TERM = (
    """SELECT
        ...
        HAVING DATE(temperatures.date) > (SELECT MAX(DATE(temperatures.date)) - (%s) FROM temperatures WHERE room_id = (%s));"""
)
...
def get_room_term(room_id, term):
    ...
    with connection:
        with connection.cursor() as cursor:
            ...
            cursor.execute(ROOM_TERM, (room_id, terms[term], room_id))
